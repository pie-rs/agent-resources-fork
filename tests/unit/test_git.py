...
class TestRaiseCloneError:
    ...
    def test_could_not_resolve_host(self):
        with patch.dict(os.environ, {"GITHUB_TOKEN": "tok"}, clear=True):
            with pytest.raises(AgrError, match="could not resolve host"):
                raise_clone_error(
    ...
    def test_unrecognized_error_with_token_raises_agr_error(self):
        ...
        with patch.dict(os.environ, {"GITHUB_TOKEN": "tok"}, clear=True):
            with pytest.raises(AgrError, match="Failed to clone"):
                raise_clone_error(
    ...
    def test_error_message_includes_source_name(self):
        with patch.dict(os.environ, {"GITHUB_TOKEN": "tok"}, clear=True):
            with pytest.raises(AgrError, match="'github'"):
                raise_clone_error(
                    "fatal: unknown error",
                    "owner",
                    "repo",
    ...
    def test_repo_not_found_includes_owner_and_repo(self):
        with patch.dict(os.environ, {"GITHUB_TOKEN": "tok"}, clear=True):
            with pytest.raises(RepoNotFoundError, match="alice/my-repo"):
                raise_clone_error(
                    "ERROR: Repository not found.",
                    "alice",
                    "my-repo",
